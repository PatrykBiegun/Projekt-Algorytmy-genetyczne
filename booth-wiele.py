...
def update(frame):
    ...
    if fixed_minima:
        ...
        ax.scatter(fixed_population[:, 0], fixed_population[:, 1], Booth(fixed_population.T), color='b')  # Plot found minima as blue points
...
# Parametry algorytmu
num_individuals = 50
bounds = (-10, 10)  # Typowy zakres dla funkcji Booth
sigma_share = 1.0
generations = 100
num_minima = 5
tournament_size = 5
...
best_individual = population[np.argmin(Booth(population.T))]
best_fitness = Booth(best_individual)
print(f'Final best solution: {best_individual}, Fitness: {best_fitness}')
