...
# Definicja funkcji Lisie Nory (F5)
def Lisie_Nory(v):
    x, y = v
    return np.square(x) + np.square(y) - 10 * (np.cos(2 * np.pi * x) + np.cos(2 * np.pi * y)) + 10
...
def calculate_fitness_with_penalty(population, sigma_share, found_minima):
    fitness_scores = np.array([Lisie_Nory(ind) for ind in population])
...
def identify_unique_minima(population, distance_threshold):
    ...
    for ind in population:
        if all(np.linalg.norm(ind - unique_ind[0]) >= distance_threshold for unique_ind in unique_minima):
            unique_minima.append((ind, Lisie_Nory(ind)))
...
def plot_current_state(ax, population, found_minima, fixed_minima):
    ...
    if fixed_minima:
        ...
        ax.scatter(fixed_population[:, 0], fixed_population[:, 1], Lisie_Nory(fixed_population.T), color='yellow', s=200, edgecolor='black', linewidth=2, label='Found Minima')
    ax.scatter(population[:, 0], population[:, 1], Lisie_Nory(population.T), color='r')
    ax.set_xlabel('X axis')
    ax.set_ylabel('Y axis')
    ax.set_zlabel('Function value')
    ax.set_title('3D Visualization of Lisie Nory Function Evolution')
    ax.legend()
...
X, Y = np.meshgrid(x, y)
Z = np.array([[Lisie_Nory([xi, yi]) for xi in x] for yi in y])
