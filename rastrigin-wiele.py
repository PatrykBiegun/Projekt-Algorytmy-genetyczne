...
# Definicja funkcji Rastrigina
def Rastrigin(v):
    A = 10
    return A * 2 + (v[0]**2 - A * np.cos(2 * np.pi * v[0])) + (v[1]**2 - A * np.cos(2 * np.pi * v[1]))
...
def identify_unique_minima(population, distance_threshold):
    ...
    for ind in population:
        if all(np.linalg.norm(ind - unique_ind[0]) >= distance_threshold for unique_ind in unique_minima):
            unique_minima.append((ind, Rastrigin(ind)))
...
def init():
    ...
    ax.set_title('3D Visualization of Rastrigin Function Evolution')
    return ax,
...
def update(frame):
    ...
    if fixed_minima:
        ...
        ax.scatter(fixed_population[:, 0], fixed_population[:, 1], Rastrigin(fixed_population.T), color='yellow', s=200, edgecolor='black', linewidth=2, label='Found Minima')
    ax.scatter(population[:, 0], population[:, 1], Rastrigin(population.T), color='r')
...
# Parametry algorytmu
num_individuals = 50
bounds = (-7.12, 7.12)  # Typowy zakres dla funkcji Rastrigina
sigma_share = 1.5
generations = 200
num_minima = 4
tournament_size = 4
